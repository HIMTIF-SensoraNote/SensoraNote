    ...
    <MobileLayout hideMobileTopNav={true}>
      <div className="min-h-screen bg-slate-50/50 dark:bg-[#13111C] font-['Manrope'] pb-12">
        ...
        <div className="sticky top-0 z-20 bg-white/95 dark:bg-[#13111C]/95 backdrop-blur-md border-b border-slate-100 dark:border-white/5">
          <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center gap-4">
            <button 
              onClick={() => navigate(-1)} 
        ...
        <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
          {/* SEARCH BAR */}
          <div className="w-full">
            <div className="relative group max-w-2xl">
              ...
              <input 
                ...
                className="w-full bg-white dark:bg-[#1C1A29] border border-slate-200/80 dark:border-white/10 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-medium text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-primary/20 focus:border-primary shadow-xs transition-all"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            {/* LEFT COLUMN: Account Center & Branding */}
            <div className="lg:col-span-4 space-y-6">
              {/* ACCOUNT CENTER */}
              {showAccountCenter && (
                <div className="bg-white dark:bg-[#1C1A29] border border-slate-100 dark:border-white/5 rounded-3xl p-5 shadow-xs hover:shadow-md dark:shadow-none transition-shadow duration-300">
                  ...
                  <div className="flex items-center gap-4 mb-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="font-bold text-slate-900 dark:text-slate-100 text-[15px] truncate">{user.name}</h2>
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t('settings.profile_details')}</p>
                    </div>
                  </div>

                  <p className="text-[12px] text-slate-400 dark:text-slate-500 leading-relaxed mb-4 font-medium">
                    {t('settings.account_desc')}
                  </p>

                  <div className="space-y-2 border-t border-slate-100 dark:border-white/5 pt-4">
                    <Link to="/edit-profile" className={`flex items-center justify-between text-[13px] font-semibold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors group p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 ${!matchesSearch(t('settings.personal_details')) && !matchesSearch(user.name) ? 'hidden' : ''}`}>
                      <div className="flex items-center gap-3">
                        <User size={18} className="text-slate-400 dark:text-slate-500 group-hover:text-primary transition-colors" />
                    ...
                    <Link to="/settings/security" className={`flex items-center justify-between text-[13px] font-semibold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors group p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer ${!matchesSearch(t('settings.password_security')) && !matchesSearch(user.name) ? 'hidden' : ''}`}>
                      <div className="flex items-center gap-3">
                        <Lock size={18} className="text-slate-400 dark:text-slate-500 group-hover:text-primary transition-colors" />
              ...
              {/* Branding / App Version in Left column on desktop */}
              <div className="bg-white dark:bg-[#1C1A29] border border-slate-100 dark:border-white/5 rounded-3xl p-6 hidden lg:flex flex-col items-center justify-center text-center">
                ...
                <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.25em]">{t('settings.version')} 1.2.4</p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">Platform Belajar & Catatan Terstruktur</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              {/* Activity Section */}
              {showActivity && (
                <div className="bg-white dark:bg-[#1C1A29] border border-slate-100 dark:border-white/5 rounded-3xl overflow-hidden shadow-xs">
                  <div className="px-6 pt-5 pb-2">
                    <h3 className="text-[12px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                      {t('settings.your_activity')}
                    </h3>
                  </div>
              ...
              {/* Login / Danger Section */}
              {showLogin && (
                <div className="bg-white dark:bg-[#1C1A29] border border-slate-100 dark:border-white/5 rounded-3xl overflow-hidden shadow-xs">
                  <div className="px-6 pt-5 pb-2">
                    <h3 className="text-[12px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                      {t('settings.login_section')}
                    </h3>
                  </div>
                  <div className="divide-y divide-slate-50 dark:divide-white/5">
                     <SettingRow 
              ...
              {/* Footer Branding on Mobile */}
              <div className="lg:hidden px-6 py-6 flex flex-col items-center">
