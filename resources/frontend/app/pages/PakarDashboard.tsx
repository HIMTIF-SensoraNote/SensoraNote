    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white dark:bg-[#1C1A29] w-full max-w-lg rounded-[24px] shadow-xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 dark:border-white/5">
                {/* Modal Header */}
                <div className={`p-6 md:p-8 pb-4 flex justify-between items-start ${type === 'approve' ? 'bg-blue-50/50 dark:bg-blue-500/10' : type === 'cancel' ? 'bg-rose-50/50 dark:bg-rose-500/10' : 'bg-rose-50/50 dark:bg-rose-500/10'}`}>
                    <div className="flex gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm ${type === 'approve' ? 'bg-blue-600 text-white' : 'bg-rose-600 text-white'}`}>
                ...
                <div className="p-6 md:p-8 space-y-8">
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-end">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-md ${
                                    ...
                                    rating <= 3 ? 'bg-blue-50 text-blue-600' : 
                    ...
                    <div className="space-y-3">
                        ...
                        <textarea
                            ...
                            className="w-full h-28 p-4 bg-white dark:bg-[#13111C] border border-slate-200 dark:border-white/10 rounded-xl text-[14px] font-medium text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all resize-none shadow-sm dark:shadow-none"
                        />
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={handleFormSubmit} disabled={isSubmitting || (type !== 'approve' && !reason.trim())} className={`flex-[2] py-3.5 rounded-xl font-['Lexend_Deca'] font-bold text-[12px] uppercase tracking-wider text-white shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50 ${type === 'approve' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-rose-600 hover:bg-rose-700'}`}>
...
export default function PakarDashboard() {
    ...
    const stats = [
        ...
        { label: t('pakar_dashboard.approved'), value: verifiedNotes.length, color: "text-blue-600", icon: CheckCircle, sparkColor: "#6366f1" },
                        ...
                        <div className="flex flex-row items-center justify-between gap-6 mb-8 pb-6 border-b border-gray-200 dark:border-white/10">
                            <div className="flex items-center gap-4">
                                <div className="relative shrink-0">
                                    ...
                                    <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center border-2 border-slate-50 dark:border-[#13111C]">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4 mb-4">
                                <div className="relative w-full sm:max-w-xs group">
                                    ...
                                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t('pakar_dashboard.search_placeholder')} className="w-full pl-9 pr-4 py-2 bg-gray-100/50 dark:bg-white/5 border-none rounded-full text-[14px] text-gray-900 dark:text-gray-100 placeholder:text-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all" />
                                </div>
                                    ...
                                    {[
                                        ...
                                        return (
                                            <button key={tab.id} onClick={() => setFilter(tab.id as any)} className={`relative flex-1 sm:flex-none px-4 py-3 text-[14px] font-bold transition-colors ${active ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5"}`}>
                                                {tab.label}
                                                {tab.id === 'pending' && pendingNotes.length > 0 && <span className="ml-1.5 text-blue-500">({pendingNotes.length})</span>}
                                                {active && <div className="absolute bottom-0 left-0 right-0 h-1 bg-blue-500 rounded-t-full" />}
                                            </button>
                                        );
                                    })}
                                </div>
                                                    ...
                                                    renderActions={(n) => (
                                                        <>
                                                            {n.isValidated ? (
                                                                ...
                                                                <>
                                                                    <button onClick={() => handleActionClick(n, "approve")} className="px-3 py-1.5 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full font-['Lexend_Deca'] font-bold text-[10px] hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-all flex items-center gap-1.5 uppercase tracking-widest border border-blue-100 dark:border-blue-500/20"><CheckCircle size={14} /> {t('pakar_dashboard.approve')}</button>
